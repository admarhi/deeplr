...
#'
#' \code{translate} translates texts between different languages using DeepL 
#' API Pro. Use \code{available_languages} to list all supported languages. An 
#' authentication key is required to use this service. The service costs 
#' depending on the number of translated characters.
#'
...
#' exceed 30kbytes.
#' @param target_lang target language of the translation. If input is of length 
#' 1, all elements are translated into the same language.
...
#' @export
#' 
#' @importFrom purrr map2_chr map2_dfr pmap_chr
#'
#' @examples
#' \dontrun{
...
#' translate(
#'   text = "I like to translate texts.", 
#'   target_lang = c("FR", "DE", "IT"), 
#'   auth_key = "x"
#' )
#'
...
translate <- function(
  ...
  if (length(target_lang) > 1 & length(text) == 1) {
    text <- rep(text, length(target_lang))
  }

  # Without source_lang
  if (is.null(source_lang)) {
    if (!get_detect) {
      translation_vec <- map2_chr(
        text,
        target_lang,
        translate_wh,
